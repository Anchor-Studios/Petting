...
public class ReturnNotTeleportOwnerProcedure {
	public static boolean execute(LevelAccessor world, Entity entity) {
		if (entity == null)
			return false;
		if (!(null == entityFromStringUUID(entity.getData(PettingModVariables.PLAYER_VARIABLES).petsettingsuuid, (Level) world))) {
			return !entityFromStringUUID(entity.getData(PettingModVariables.PLAYER_VARIABLES).petsettingsuuid, (Level) world).getPersistentData().getBoolean("teleport");
		}
		return false;
	}

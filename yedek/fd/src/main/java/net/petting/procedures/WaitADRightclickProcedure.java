...
public class WaitADRightclickProcedure {
	public static void execute(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Entity entity) {
		...
		if (!((entity instanceof LivingEntity _teamEnt && _teamEnt.level().getScoreboard().getPlayersTeam(_teamEnt instanceof Player _pl ? _pl.getGameProfile().getName() : _teamEnt.getStringUUID()) != null
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal(("\u00A7eRightclick to a pet to set it's attack distance to " + DoubleArgumentType.getDouble(arguments, "name") + ".")), false);
			entity.getPersistentData().putDouble("adupdate", (DoubleArgumentType.getDouble(arguments, "name")));
			entity.getPersistentData().putBoolean("adwait", true);
			PettingMod.queueServerWork(350, () -> {
				if (entity.getPersistentData().getBoolean("adwait")) {
					if (entity instanceof Player _player && !_player.level().isClientSide())
						_player.displayClientMessage(Component.literal("\u00A74Attack distance changing request cancelled, waited too long."), false);
